...
def _resolve_kzt_php_script(cfg: dict[str, Any]) -> Path | None:
	...
	if cli.is_file():
		return cli
	# 2) копия рядом с парсером (запуск php на хосте, БД доступна как 127.0.0.1)
...
def resolve_rub_per_kzt(cfg: dict[str, Any]) -> float | None:
	"""Сколько рублей за 1 KZT по курсу Bitrix; при сбое — fallback из config."""
...
def parse_kzt_from_price_raw(raw: str) -> float | None:
	"""Число тенге из строки витрины («4 200 ₸», «99 999»). Если не угадали — None."""
...
def effective_kzt_amount(pkzt: float | None, price_raw: str) -> float | None:
	"""Сумма в KZT: с карточки (price_kzt) или из строки списка."""
	if pkzt is not None:
		try:
			v = float(pkzt)
			if v > 0:
...
def export_products_csv_legacy(
	...
	Цена: в ₽ по сумме в KZT (price_kzt с карточки или разбор price_raw со списка) и курсу Bitrix.
	Если курса нет или сумму в KZT не удалось получить — остаётся строка price_raw.
	"""
...
def export_products_csv(
	...
	"""Обратная совместимость: старый «легаси»-экспорт."""
...
def export_products_csv_alley(
	...
	"""Бренд;Артикул;Остаток;Цена (цена в рублях по курсу KZT из Bitrix) → stock_and_price."""
	out_path.parent.mkdir(parents=True, exist_ok=True)
