...
def import_structure(*, snapshot_path: str, resume: bool = True) -> dict[str, Any]:
    ...
    try:
        nav_rows = snap.execute(
            """
            SELECT DISTINCT root_arib, arib, aria, slug, rel, title, depth, path_json
            FROM api_nodes
            WHERE rel <> 'assembly'
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            if not resume:
                cur.execute(
                    """
                    TRUNCATE oem_diagram_hotspots, oem_assembly_parts, oem_diagrams,
                      oem_details_pages, oem_assemblies, oem_variants, oem_nav_nodes,
                      oem_parts, oem_crawl_checkpoints RESTART IDENTITY CASCADE
                    """
                )
                conn.commit()
            ...
            if resume:
                cur.execute("SELECT id, variant_key FROM oem_variants")
            ...
            if resume:
                cur.execute("SELECT variant_id, assembly_key FROM oem_assemblies")
            ...
            cur.execute("SELECT COUNT(*) AS c FROM oem_nav_nodes")
...
def _flush_assemblies(cur: Any, batch: list[tuple[Any, ...]]) -> list[int]:
    ...
    cur.executemany(
        ...
        INSERT INTO oem_assemblies(
          variant_id, root_arib, assembly_key, aria, slug, title, path_json, sort_order
