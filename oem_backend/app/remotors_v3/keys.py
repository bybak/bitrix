...
from .constants import ROOT_ARIB_CODES
...
def key_from_str(value: str) -> tuple[Any, ...]:
...
def resolve_root_arib(arib: str | None, fallback: str) -> str:
    code = (arib or fallback).strip().upper()
    if code in ROOT_ARIB_CODES:
        return code
    return fallback


def variant_key_from_path(root_arib: str, path: list[str]) -> str:
    """Variant identity = root + full browse path excluding assembly title."""
    if not path:
...
def variant_fields(root_arib: str, path: list[str]) -> dict[str, Any]:
    ...
    browse_line = variant_path[0] if root_arib == "BRP" and variant_path else None
    return {
        "root_arib": root_arib,
        "variant_key": variant_key_from_path(root_arib, path),
