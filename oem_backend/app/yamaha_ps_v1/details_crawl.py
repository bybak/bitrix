...
from app.yamaha_ps_v1.client import (
    brand_from_path,
    extract_diagram_url,
    fetch_details_html,
...
def _thread_http_client() -> Any:
    ...
    if client is None:
        client = make_client(max_connections=_http_max_connections)
...
def _pg_crawl_conn() -> Iterator[psycopg.Connection]:
    conn = psycopg.connect(get_settings().yamaha_database_dsn, row_factory=dict_row, autocommit=False)
    try:
...
def _claim_batch_pg(*, phase: str, force: bool, batch_size: int = 1) -> list[dict[str, Any]]:
    ...
    with _pg_crawl_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(select_sql, (ROOT_ARIB, list(statuses), batch_size))
            rows = cur.fetchall()
            if not rows:
                return []
            for row in rows:
                cur.execute(
                    ...
                    SET {status_col} = 'running', updated_at = now()
                    WHERE id = %s
                    """,
                    (row["details_page_id"],),
                )
...
def _html_storage_path(root_arib: str, assembly_id: int) -> Path:
    ...
    return Path(settings.asset_root).parent / HTML_SUBDIR / root_arib / f"{assembly_id}.html"


def _image_storage_path(root_arib: str, assembly_id: int, ext: str = ".png") -> Path:
    settings = get_settings()
    return Path(settings.asset_root) / ILLUST_SUBDIR / root_arib / str(assembly_id) / f"diagram{ext}"


def _resolve_brand(row: dict[str, Any]) -> Any:
    path = row.get("path_json") or []
    ...
    code = (payload or {}).get("partstream_brand")
    if code:
        from app.yamaha_ps_v1.client import brand_context

        return brand_context(str(code))
    raise RuntimeError(f"Cannot resolve PartStream brand for assembly={row.get('assembly_id')}")
...
def _mark_image_placeholder(assembly_id: int, original_url: str) -> None:
    """No real diagram (PartStream NoImage.*) — treat as successful empty image."""
    with _pg_crawl_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                UPDATE oem_details_pages SET
                  ...
                  image_path = NULL,
                  image_status = 'ok',
                  error_message = 'no-image placeholder',
                  updated_at = now()
                WHERE assembly_id = %s
                """,
                (original_url, assembly_id),
            )
...
def _process_html_row(
    ...
    *,
    ...
    try:
        ...
        html = fetch_details_html(_thread_http_client(), brand, slug, delay_ms=80, jitter_ms=120)
        if "ariRequestErrorMsg" in html:
            raise RuntimeError("PartStream GetDetails returned request error")
        ...
        if is_retryable_http_error(exc) or "Incapsula" in str(exc):
            with stats_lock:
                stats["retries"] += 1
...
def _process_image_row(
    ...
    *,
    ...
    if is_placeholder_diagram_url(image_url):
        _mark_image_placeholder(assembly_id, image_url)
        with stats_lock:
            stats["skipped"] += 1
    ...
    try:
        ...
        response = client.get(
            image_url,
            headers={"User-Agent": HTTP_USER_AGENT, "Referer": brand.ariv},
        )
        response.raise_for_status()
        # Some responses 200-redirect into a NoImage asset — treat as placeholder.
        final_url = str(response.url) if response.url else image_url
        if is_placeholder_diagram_url(final_url) or is_placeholder_diagram_url(image_url):
            _mark_image_placeholder(assembly_id, final_url or image_url)
            with stats_lock:
                stats["skipped"] += 1
            progress.advance(f"skip no-image placeholder assembly={assembly_id}")
            return
        ...
        # 404 on NoImage.gif/Max (and similar) is not a real failure.
...
def crawl_details(
    *,
    ...
    if concurrency is None:
        concurrency = DEFAULT_HTML_CONCURRENCY if phase == "html" else DEFAULT_IMAGE_CONCURRENCY
    concurrency = max(1, int(concurrency))
    global _http_max_connections
    _http_max_connections = max(200, concurrency + 20)

    _ensure_pg_crawl_schema()
    if worker_id == 0:
        reset = _reset_stale_running(phase)
        if reset:
            print(f"[yamaha-ps-crawl-{phase}] reset stale running={reset}", flush=True)
    elif workers > 1:
        time.sleep(worker_id * 2)

    worker_label = f"yamaha-ps-crawl-{phase}" if workers == 1 else f"yamaha-ps-crawl-{phase}-w{worker_id}/{workers}"
    if concurrency > 1:
        worker_label = f"{worker_label}x{concurrency}"
    progress = ProgressReporter(total=0, label=worker_label)
    progress.enable_thread_safe()
    stats = {"ok": 0, "errors": 0, "retries": 0, "skipped": 0, "circuit_open": False}
    stats_lock = threading.Lock()
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as executor:
        while True:
            ...
            with stats_lock:
                before_ok = stats["ok"]
                before_fail = stats["errors"] + stats["retries"]
            futures = [
            ...
            with stats_lock:
                ok_delta = stats["ok"] - before_ok
                fail_delta = (stats["errors"] + stats["retries"]) - before_fail
            if ok_delta > 0:
                consecutive_failures = 0
            elif fail_delta > 0:
                consecutive_failures += fail_delta
                if consecutive_failures >= CIRCUIT_BREAKER_ERRORS:
                    stats["circuit_open"] = True
                    progress.advance(
                        f"CIRCUIT OPEN after {consecutive_failures} consecutive failures — stop crawl"
                    )
                    break
            ...
            if time.monotonic() - last_tick >= PROGRESS_INTERVAL_SEC:
                with stats_lock:
                    progress.tick(
                        f"ok={stats['ok']} errors={stats['errors']} retries={stats['retries']}"
                    )
                last_tick = time.monotonic()
