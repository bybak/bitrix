...
from app.yamaha_v1.parse_responses import parse_catalog_text
...
def _process_parse_row(
    ...
    *,
    ...
    try:
        ...
        parsed = parse_catalog_text(response, image_url=row.get("image_url"))
...
def parse_details(
    *,
    ...
    if worker_id == 0:
        ...
        if reset:
            print(f"[yamaha-parse] reset stale running={reset}", flush=True)
    elif workers > 1:
        time.sleep(worker_id * 2)

    worker_label = "yamaha-parse" if workers == 1 else f"yamaha-parse-w{worker_id}/{workers}"
    if concurrency > 1:
