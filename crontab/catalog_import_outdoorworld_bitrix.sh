#!/usr/bin/env bash
# Импорт остатков/цен в Bitrix из CSV OutdoorWorld (внутри контейнера путь — см. volume в docker-compose).
set -euo pipefail

CONTAINER=bitrix_php
IN_CONTAINER_FILE=/var/www/html/_catalog_scrape/outdoorworld/stock_and_price.csv
LOG_PREFIX="[outdoorworld_bitrix]"
