	...
	BX.Sale.OrderAjaxComponent = {
		...
		editDeliveryBlock: function(active)
		{
			if (!this.deliveryBlockNode || !this.deliveryHiddenBlockNode)
				return;
			// Виртуальный eDost: даже без служб доставки в каталоге нужны местоположение и #mf-edost-*.
			if (!this.result.DELIVERY)
				this.result.DELIVERY = [];
		...
		editFadePropsContent: function(node)
		{
			...
			// Motor-Force: в свёрнутом «Покупателе» всегда дополняем список полями DELIVERY_*,
			// даже если PROPS_FADE_LIST_* задан без них — иначе красная ошибка без видимого «Адреса доставки».
			try
			{
		...
		isValidPropertiesBlock: function(excludeLocation)
		{
			...
			for (i = 0; i < props.length; i++)
			{
				...
				// Motor-Force: строка справочного LOCATION скрыта под Nominatim — не валидировать пустой UI.
