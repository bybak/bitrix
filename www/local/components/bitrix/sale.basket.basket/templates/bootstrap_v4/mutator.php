...
foreach ($this->basketItems as $row)
{
	...
	{
		$price = function_exists('mf_round_price') ? mf_round_price($rowData['PRICE'] * $rowData['MEASURE_RATIO']) : PriceMaths::roundPrecision($rowData['PRICE'] * $rowData['MEASURE_RATIO']);
		...
		$fullPrice = function_exists('mf_round_price') ? mf_round_price($rowData['FULL_PRICE'] * $rowData['MEASURE_RATIO']) : PriceMaths::roundPrecision($rowData['FULL_PRICE'] * $rowData['MEASURE_RATIO']);
		...
		$discountPrice = function_exists('mf_round_price') ? mf_round_price($rowData['DISCOUNT_PRICE'] * $rowData['MEASURE_RATIO']) : PriceMaths::roundPrecision($rowData['DISCOUNT_PRICE'] * $rowData['MEASURE_RATIO']);
	...
	if (!empty($rowData['PROPS']) && is_array($rowData['PROPS']) && function_exists('mf_basket_dedupe_props_for_display'))
	{
