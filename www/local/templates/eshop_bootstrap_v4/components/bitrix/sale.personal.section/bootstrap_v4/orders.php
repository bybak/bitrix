...
$mfStatusFilterKeys = ['mf_order_status', 'mf_payment_status', 'mf_shipment_status'];
$mfHasStatusFilter = false;
foreach ($mfStatusFilterKeys as $mfFilterKey)
{
	if (trim((string)($_REQUEST[$mfFilterKey] ?? '')) !== '')
	{
		$mfHasStatusFilter = true;
		break;
	}
}

if (!isset($_REQUEST['show_all']) && !isset($_REQUEST['filter_history']) && !$mfHasStatusFilter)
{
	$_REQUEST['show_all'] = 'Y';
}

$ordersPerPage = (int)($arParams['ORDERS_PER_PAGE'] ?? 20);
if ($mfHasStatusFilter && $ordersPerPage < 200)
{
	$ordersPerPage = 200;
}
...
$APPLICATION->IncludeComponent(
	...
	[
		...
		'SAVE_IN_SESSION' => 'N',
		'ORDERS_PER_PAGE' => (string)$ordersPerPage,
