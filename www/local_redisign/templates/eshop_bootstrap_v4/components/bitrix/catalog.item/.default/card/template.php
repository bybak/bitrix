...
// Витринная цена: только склады с остатком.
$mfDynPrice = null;
	...
	<div class="mf-pcard__body">
		...
		<div class="mf-pcard__meta">
			<div class="mf-pcard__price product-item-price-container" data-entity="price-block">
				...
				<?php endif; ?>
				<?php
				$mfHasPriceForCard = false;
				$mfPriceMarkup = '';
				if ($canBuy && $mfDynPrice !== null)
				{
					$mfHasPriceForCard = true;
					$mfDp = round((float)$mfDynPrice, 1);
					$mfDd = ((int)round(abs($mfDp) * 10) % 10 === 0) ? 0 : 1;
					$mfPriceMarkup = function_exists('mf_format_display_price_rub')
				...
				{
					$mfHasPriceForCard = true;
					$mfPriceMarkup = function_exists('mf_format_display_price_rub') && isset($price['RATIO_PRICE'])
						? ('От ' . htmlspecialcharsbx(mf_format_display_price_rub((float)$price['RATIO_PRICE'])))
						: ('От ' . $price['PRINT_RATIO_PRICE']);
				}
				?>
				<span class="mf-pcard__cur product-item-price-current<?= $mfHasPriceForCard ? '' : ' mf-pcard__cur--ask' ?>" id="<?=$itemIds['PRICE']?>">
					<?= $mfHasPriceForCard ? $mfPriceMarkup : 'Цена по запросу' ?>
				</span>
			</div>
		</div>
