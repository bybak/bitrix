...
if ($isHome)
{
	// Старый дизайн главной (mf-mainpage.css/js) больше не подгружаем — используем mf-home.css.
	$APPLICATION->AddHeadString(
		'<link rel="stylesheet" href="' . htmlspecialcharsbx($mfAssetVer(SITE_TEMPLATE_PATH . '/mf-home.css')) . '">',
		true
	);
}
if ($isPosts)
...
<head>
	...
	// --- Premium redesign layer (loaded LAST so it overrides all mf-*.css) ---
	echo '<link rel="preconnect" href="https://fonts.googleapis.com">' . "\n";
	echo '<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>' . "\n";
	echo '<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Manrope:wght@400;500;600;700;800&family=Bebas+Neue&display=swap">' . "\n";

	$redesignCss = $mfAssetVer(SITE_TEMPLATE_PATH . "/mf-redesign.css");
	echo '<link rel="stylesheet" href="' . htmlspecialcharsbx($redesignCss) . '">' . "\n";

	// GSAP CDN (animations) — graceful: redesign.js no-ops if gsap missing.
	echo '<script src="https://cdn.jsdelivr.net/npm/gsap@3.12.5/dist/gsap.min.js" defer></script>' . "\n";
	echo '<script src="https://cdn.jsdelivr.net/npm/gsap@3.12.5/dist/ScrollTrigger.min.js" defer></script>' . "\n";

	$redesignJs = $mfAssetVer(SITE_TEMPLATE_PATH . "/mf-redesign.js");
	echo '<script src="' . htmlspecialcharsbx($redesignJs) . '" defer></script>' . "\n";
	?>
</head>
