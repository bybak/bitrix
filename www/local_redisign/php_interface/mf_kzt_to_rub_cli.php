<?php
/**
 * CLI: сумма в KZT → рубли по курсам модуля «Валюты» (явно в RUB, не в «базовую» валюту сайта).
 * Лежит в DOCUMENT_ROOT — удобно вызывать из контейнера php (хост БД = mysql).
 *
 * Пример (из каталога с docker-compose.yml):
...
if (\Bitrix\Main\Loader::includeModule('currency') && class_exists(\CCurrencyRates::class)) {
	$r = \CCurrencyRates::ConvertCurrency($kzt, 'KZT', $target);
	if (is_finite($r) && (float)$r > 0) {
		$rub = round((float)$r, 2);
	}
}

// Если валюты RUB нет или курс не сходится — старый путь: в «базовую» (на старых конфигах это мог быть RUB).
