...
if (!function_exists('mf_mf_catalog_section_id_by_code'))
{
	/**
	 * ID раздела каталога (IBLOCK 4) по символьному коду — баннеры главной и т.п.
	 */
	function mf_mf_catalog_section_id_by_code(string $code, int $iblockId = 4): int
	{
		static $byCode = null;
		...
		{
			$byCode = [];
			if (!class_exists('CIBlockSection') || !\Bitrix\Main\Loader::includeModule('iblock'))
			{
				return 0;
			}
			...
			while ($row = $rs->Fetch())
			{
				...
				if ($c !== '')
				{
					$byCode[$c] = (int)($row['ID'] ?? 0);
				}
			}
		}

		return (int)($byCode[$code] ?? 0);
	}
}

if (!function_exists('mf_mf_home_prokat_slider_src'))
{
	/**
	 * Баннер «Прокат»: /upload/mf_home_slider_prokat.jpg или запас — раздел квадро.
	 */
	function mf_mf_home_prokat_slider_src(): string
	{
		$doc = (string)($_SERVER['DOCUMENT_ROOT'] ?? '');
		if ($doc !== '')
		{
			$local = $doc . '/upload/mf_home_slider_prokat.jpg';
			if (is_file($local))
			{
				return '/upload/mf_home_slider_prokat.jpg?v=' . (int)filemtime($local);
			}
		}
		$id = function_exists('mf_mf_catalog_section_id_by_code')
			? mf_mf_catalog_section_id_by_code('zapchasti-dlya-kvadrotsiklov', 4)
			: 0;

		return $id > 0 && function_exists('mf_mf_section_img_url')
			? (string)mf_mf_section_img_url($id)
			: (function_exists('mf_mf_placeholder_img_url') ? (string)mf_mf_placeholder_img_url() : '');
	}
}
